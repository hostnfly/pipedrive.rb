...
RSpec.describe ::Pipedrive::V2::Deal do
  subject { described_class.new('token') }

  describe '#entity_name' do
    it { expect(subject.entity_name).to eq('deals') }
  end

  describe '#build_url' do
    it 'uses /api/v2/deals' do
      expect(subject.build_url([])).to eq('/api/v2/deals')
    end

    it 'appends id when provided' do
      expect(subject.build_url([nil, 42])).to eq('/api/v2/deals/42')
    end
  ...
  describe '#create' do
    it 'POSTs to /api/v2/deals with x-api-token header' do
      ...
      expect(subject.create(title: 'New deal', owner_id: 1)).to be_success
    end
  end

  describe '#find_by_id' do
    it 'GETs /api/v2/deals/:id with x-api-token header' do
      stub_request(:get, 'https://api.pipedrive.com/api/v2/deals/1')
        .with(headers: { 'x-api-token' => 'token' })
        .to_return(status: 200, body: { success: true, data: {} }.to_json, headers: {})
  ...
  describe '#update' do
    it 'PATCHes /api/v2/deals/:id with x-api-token header' do
      stub_request(:patch, 'https://api.pipedrive.com/api/v2/deals/1')
        .with(headers: { 'x-api-token' => 'token' })
        .to_return(status: 200, body: { success: true, data: {} }.to_json, headers: {})
      expect(subject.update(1, title: 'Updated deal')).to be_success
    end
  end

  describe '#delete' do
    it 'DELETEs /api/v2/deals/:id with x-api-token header' do
      stub_request(:delete, 'https://api.pipedrive.com/api/v2/deals/1')
        .with(headers: { 'x-api-token' => 'token' })
